...
export default function EditExpenseProfileComponent({
    expenseProfile,
}: {
    expenseProfile: TExpenseProfile | null;
}) {
    const [selectedColor, setSelectedColor] = useState<string>(
        expenseProfile?.color || "#FFFFFF",
    );

    const editExpenseProfileMutation = useMutation({
        mutationKey: ["editExpenseProfileMutation"],
        mutationFn: async (
            editedExpenseProfile: z.infer<typeof formSchema>,
        ) => {
            const response = await axios.put(
                "/api/expenseProfiles?expenseProfileId=" + expenseProfile?.id,
                editedExpenseProfile,
            );
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
            name: expenseProfile?.name || "",
            color: expenseProfile?.color || "#FFFFFF",
            budget: expenseProfile?.budget || 0,
        },
    });
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        editExpenseProfileMutation.mutate(values);
    }
    ...
    return (
        <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
                <DialogTitle>
                    Edit <span className="text-blue-600">Expense</span> profile
                </DialogTitle>
            </DialogHeader>
